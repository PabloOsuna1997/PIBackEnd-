...
module.exports = function (router) {
    router.put('/:correo/:venta', async (req, res) => {
        try {
            //verificacion que haya los productos necesarios 
            /* const verificacion = await db.query('SELECT * FROM DETALLE_VENTA WHERE venta = ?', [req.params.venta]);
             for (var index = 0; index < verificacion.length; index++) {
                 var element = Object.assign({}, verificacion[index]);
                 ...
                 if(di.cantidad_nueva >= 1){
                     //si es mayor o igual a 1 es que puedo disminuir un elemento.
            ...
            if (result.affectedRows > 0) {
                ...
                for (var index = 0; index < productos.length; index++) {
                    ...
                    var deta_ = await db.query('SELECT * FROM DETALLE_INVENTARIO  WHERE inventario = ? AND producto = ?',
                        [element.codigo_inventario, element.producto]);
                    deta = Object.assign({}, deta_[0]);

                    if (deta.cantidad_nueva >= 1) {
                        //si es mayor o igual a 1 es que puedo disminuir un elemento.
